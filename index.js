...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`918617738028@s.whatsapp.net`]
...
cmhit.push(command)
  ...
  /*const reply = (teks) => {
      Pga ff.sendMessage(from, teks, text, { quoted: mek });
    };*/
    const textImg = (teks) => {
...
/*const reply = (teks) => {
			Pga ff.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/918617738028text=hello`}}})
		}*/
...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Xeon Bot Inc.`, "footerText": "*_PGA~BOT_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`PGA~BOT`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
...
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Xeon`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/918617738028text=hello bot owner Pga Ff`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nPGA~BOT. (@pga_yt) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/918617738028`,
                "mediaType": "10",
                "mediaUrl": `wa.me/918617738028`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/918617738028`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "918617738028@s.whatsapp.net";
...
*Thank you for registering in the PGA~BOT Database*
...
*「 𝐃𝐨𝐠𝐞 𝐁𝐨𝐭 」*`
	    ...
	    // console.log(e)
			...
			reply(`*Thank you for being registered PGA~BOT*`)
		}, 2000)
        break
...
groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "918617738028@s.whatsapp.net"
        dtod = '918617738028'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'bugreport':
              ...
              xeon.sendMessage('918617738028@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
break
case 'hi':
reply('HI BRO/SIS TYPE #menu FOR MENU')
break
case 'hue hue hue':
reply('yes no time')
break
...
case 'iggw':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
  reply('https://www.instagram.com/pga_yt/')
break
...
case 'buglink':
            ...
            "externalAdReply": {
                ...
                "sourceUrl": "https://youtube.com/pgaff"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
        xeon.toggleDisappearingMessages(from, 0)
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `PGA~BOT`, [
          {
            buttonId: `command`,
            buttonText: {
...
case 'script':
		case 'sc':
		case 'sourcecode':
		xeon.sendMessage(from, { text: "https://github.com/PGA-FF/pgabot", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
if (!q) return reply(`Example : ${prefix + command} 2007 03 27`)
const zodiak = [
...
case 'setprefix':
    ...
    Follower: ${tod.result.user_followers}`;
        pga.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;*/
  ...
  case 'writebook':            
            if (args.length == 0) return reply(`example: ${prefix + command} Pga ff`)
            reply(mess.wait)
            teks = args.join(" ")
...
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              ...
              buttonsMessage = {footerText:'PGA~BOT', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, `PGA~BOT`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 PGA~BOT BROADCASTING 」*\n\n${body.slice(4)}`})
}
...
case 'writelist':
  ...
  🌿 ${prefix}write Pga ff
  🌿 ${prefix}folioright Pga ff
  🌿 ${prefix}folioleft Pga ff
  🌿 ${prefix}writedown Pga ff
  🌿 ${prefix}writeleft Pga ff`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
...
buttons = [{buttonId: `pinterest`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'Pga ff', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await xeon.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              xeon.relayWAMessage(prep)
        break;
